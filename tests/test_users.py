...
class AllTests(unittest.TestCase):
  ...
  def test_users_can_register(self):
    new_user = User("kennedy", "kennedy@hotmail.com", bcrypt.generate_password_hash("kennedy"))
    db.session.add(new_user)
    db.session.commit()
    test = db.session.query(User).all()
    for t in test:
      t.name
    assert t.name == "kennedy"
  ...
  # test Unregistered users cannot login
  ...
  def test_users_can_login(self):
    ...
    response = self.login('kennedy', 'kennedy')
